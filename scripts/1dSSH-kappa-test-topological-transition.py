...
def single_iteration(args):
    ...
    seed_str = f"{rho}_{disorder}_{num_eigval}_{i}_{kappa:.5f}_{L}_{v}_{w}_{time_of_day}"
...
if __name__ == "__main__":
    ...
    w_start = float(parameters.get('w_start', 0.5))
    w_end = float(parameters.get('w_end', 1.5))
    w_resolution = int(parameters.get('w_resolution', 50))
    disorder = float(parameters.get('disorder', 2.0))
    ...
    w_values = np.linspace(w_start, w_end, w_resolution)
    ...
    shape_2d = (len(w_values), num_disorder_realisations)
    ...
    with Pool(processes=cpu_count, maxtasksperchild=10) as pool:
        modelToGetX =  OneDimensionalSSHBlockBasis(L,0,rho,1,v,w_start)
        X = modelToGetX.X
        sparse = False
        ...
        for j, w in enumerate(w_values):
            ...
            print(f"      Time for w {w}: {time.time() - disorder_time} seconds", flush=True)
            if retevec:
