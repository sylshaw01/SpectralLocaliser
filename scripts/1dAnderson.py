...
if __name__ == "__main__":
    ...
    with Pool(processes=cpu_count, maxtasksperchild=10) as pool:
        for i, L in enumerate(L_values):
            ...
            print(f"   Time taken for L={L}: {time.time() - L_val_time:.2f} seconds", flush=True)
    ...
    filename = f"../data/1dAnderson_L{L_start}-{L_end}_rho{rho}_kappa{kappa}_disorder{disorder_start}-{disorder_end}_numEigs{num_eigenvalues}_realizations{num_disorder_realizations}_{current_date}_results.npz"
