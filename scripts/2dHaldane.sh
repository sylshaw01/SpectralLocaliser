...
python 2dHaldane_combined.py $p "$1"
EOF
...
# read 2dHaldane
with open("2dHaldane.py", "r") as f:
    SSH_code = f.read()

Haldane_code = re.sub(r"^sys\.path\.append\(['\"]\.\.\/src['\"]\)$", "", SSH_code, flags=re.MULTILINE)

Haldane_code = re.sub(r"^from SLmodels import \*$", "", SSH_code, flags=re.MULTILINE)
...
with open("2dHaldane_combined.py", "w") as f:
    f.write(combined)
EOF

sbatch job2dHaldane.sh $1
